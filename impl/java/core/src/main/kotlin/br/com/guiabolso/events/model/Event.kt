...
sealed class Event {
    ...
    fun <T> payloadAs(clazz: Class<T>, jsonAdapter: JsonAdapter = MapperHolder.mapper): T =
        this.payload.convertTo(clazz, jsonAdapter)

    inline fun <reified T> payloadAs(jsonAdapter: JsonAdapter = MapperHolder.mapper): T =
        this.payload.convertTo(jsonAdapter)

    fun <T> identityAs(clazz: Class<T>, jsonAdapter: JsonAdapter = MapperHolder.mapper): T =
        this.identity.convertTo(clazz, jsonAdapter)

    inline fun <reified T> identityAs(jsonAdapter: JsonAdapter = MapperHolder.mapper): T =
        this.identity.convertTo(jsonAdapter)

    fun <T> authAs(clazz: Class<T>, jsonAdapter: JsonAdapter = MapperHolder.mapper): T =
        this.auth.convertTo(clazz, jsonAdapter)

    inline fun <reified T> authAs(jsonAdapter: JsonAdapter = MapperHolder.mapper): T = this.auth.convertTo(jsonAdapter)
